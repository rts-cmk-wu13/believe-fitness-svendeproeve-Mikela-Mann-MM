...
export default async function ParticipantsPage({ params }: Props){
  ...
  return (
    <main className="page-content content-wrapper pt-6" aria-label="Participants page">
        {/* Instructor header */}
        <div className="flex items-center gap-4 mb-8" aria-label="Instructor info">
            <div
            className="w-14 h-14 rounded-full flex items-center justify-center shrink-0 bg-(--brand-yellow)"
            aria-hidden="true"
            >
<User size={28} color="#0A0A0A" aria-hidden="true" />
            </div>
            <div>
...
<h2 className="text-2xl font-bold mb-2">{fitnessClass.className}</h2>
           ...
           {participants.map((user) => (
            ...
            >
              <div className="flex items-center gap-3">
                <User size={20} color="var(--brand-black)" aria-hidden="true" />
